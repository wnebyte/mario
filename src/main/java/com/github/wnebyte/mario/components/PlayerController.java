...
import com.github.wnebyte.mario.prefabs.FireballGenerator;
...
public class PlayerController extends Component {
    ...
    public void update(float dt) {
        if (playWinAnimation) {
            ...
            if (!onGround) {
                ...
                Sound sound = Assets.getSound(Context.getAssetsDir() + "/sounds/stage_clear.ogg");
                if (!sound.isPlaying()) {
                    sound.play();
                }
        ...
        if (isDead) {
            if (gameObject.transform.position.y < deadMaxHeight && deadGoingUp) {
                ...
                Window.setScene(Window.getScene().getPath(), new MarioLevelSceneInitializer());
            }
            return;
        }
        ...
        if (isKeyBeginPress(GLFW_KEY_E) && isFire() && Fireball.canSpawn()) {
            ...
            GameObject fireball = FireballGenerator.generate(pos);
        ...
        if (isKeyPressed(GLFW_KEY_SPACE) &&
            ...
            if ((onGround || groundDebounce > 0) && jumpTime == 0) {
              //  AssetFlyWeight.getSound("/sounds/jump-small.ogg").play();
                jumpTime = 28;
    ...
    public void checkOnGround() {
        ...
        onGround = Window.getScene().getPhysics().onGround(gameObject, innerPlayerWidth, yVal);
    }
    ...
    public void powerup() {
        if (isSmall()) {
            ...
            Sound sound = Assets.getSound(Context.getAssetsDir() + "/sounds/powerup.ogg");
            sound.play();
    ...
    public void die() {
        ...
        if (isSmall()) {
            ...
            rb.setIsSensor();
            isDead = true;
            Sound sound = Assets.getSound(Context.getAssetsDir() + "/sounds/mario_die.ogg");
            sound.play();
            ...
            Sound sound = Assets.getSound(Context.getAssetsDir() + "/sounds/pipe.ogg");
            sound.play();
        } else if (isFire()) {
            playerState = PlayerState.BIG;
            hurtInvincibilityTimeLeft = hurtInvincibilityTime;
    ...
    public void playWinAnimation(GameObject go) {
        if (!playWinAnimation) {
            ...
            Sound sound = Assets.getSound(Context.getAssetsDir() + "/sounds/flagpole.ogg");
            sound.play();
        }
    }
